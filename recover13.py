...
# fps : 카메라에서 전달되는 초당 프레임 수 
# frame : 이 각각의 정지 사진 1장을 '프레임'이라 부른다
# Frame Rate : 이런 사진 토막들이 1초에 몇 장 보이느냐, 즉 프레임이 보이는 속도
...
# 코덱을 사용해 영상파일 저장
# 코덱(Codec) : 영상 파일의 Encode와 Decode를 처리함으로써 우리가 화면에서 보거나 파일로 저장하는 작업을 도와줌
# 코덱 설정
fourcc = cv2.VideoWriter_fourcc(*'XVID')

# 컬러 동영상 녹화를 위해
out1 = cv2.VideoWriter('record_0.mp4', fourcc, fps, frameSize)
